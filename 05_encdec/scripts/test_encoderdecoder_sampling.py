from seq2seq_v1 import RNNEncoderDecoder as RED 
...
red =  RED(model, 2, 128, 128, len(wids), M, builder=LSTMBuilder)
 ...
 for sentence in sentences:
  ...
  if len(sentence) > 2:  
    ...
    if c%250 == 1:
         ...
         words = 0
         for _ in range(1):
             samp = red.sample(first=wids["<s>"],nchars= 30,stop=wids["</s>"])
    ...
    error = red.build_lm_graph(isent)
